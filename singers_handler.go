...
func (h *SingersHandler) RandomInsert(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		fmt.Printf("method not allowed. got %s\n", r.Method)
	...
	if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
		fmt.Printf("failed to decode request body. %s\n", err)
	...
	if body.Count < 1 {
		fmt.Printf("count must be greater than 0. got %d\n", body.Count)
	...
	if err := h.Store.BatchInsert(ctx, singers); err != nil {
		fmt.Printf("failed to insert singers. %s\n", err)
		http.Error(w, "internal server error", http.StatusInternalServerError)
		return
	}
